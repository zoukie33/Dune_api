...
exports.createPoolNotif = function (req, idToNotify, typeNotif, textNotif) {
  let query1 = 'SELECT pda.idProf FROM d_partDemandeAchat AS pda WHERE pda.idDemande = ' + idToNotify;
  ...
  req.mysql.query(query1, function(error, results, fields) {
    if(error){
      ...
      for (const k in results) {
        ...
        let query2 = 'INSERT INTO d_notifications (idUser, idToNotify, typeNotif, textNotif) VALUES ("' + idProf + '", "' + idToNotify + '", "' + typeNotif + '", "' + textNotif + '")';
        req.mysql.query(query2, function(error, resu, fields) {if (error) {return(error);}});

        console.log("Une notification a été ajoutée : [" + idProf + " - " + idToNotify + " - " + typeNotif + " - " + textNotif +"]");
      }
      return ("TRUE");
    }
  });
