 ...
 */
  ...
  req.mysql.query(query, function (error, results, fields) {
    if(error){
      ...
      if (results.length == 1) {
        ...
        tools.dSend(res, "NOK", "Table-Licence", "verif", 500, "Licence Expirée", null);
      }
    }
  });
 });
  ...
  */
  ...
  router.post('/validate/', function(req, res, next) {
    ...
    var query = "SELECT * FROM d_licencesTables AS lt WHERE lt.serial = '" + licence + "' AND lt.dateExpire > NOW() AND lt.used = 0";
    var query1 = "UPDATE d_licencesTables SET used = 1 WHERE serial = '" + licence + "'";
   ...
   req.mysql.query(query, function (error, results, fields) {
     if(error){
       ...
       if (results.length == 1) {
         req.mysql.query("UPDATE d_tables SET idSerial = " + results[0].idLicence + " WHERE idTable = " + idTable, function (error, results1, fields) {
           if(error){
             tools.dSend(res, "NOK", "Table-Licence", "Validate", 500, error, null);
           } else {
             req.mysql.query(query1, function (error, results, fields) {
               if(error){
                 ...
                 tools.dSend(res, "OK", "Table-Licence", "Validate", 200, null, "Licence validated");
               }
             });
           }
         });
       } else {
         tools.dSend(res, "NOK", "Table-Licence", "Validate", 500, "Licence Expirée", null);
       }
     }
   });
  });
