...
router.get('/nbGamesPlayed', function(req, res, next) {
  req.mysql.query(
    'SELECT COUNT(idGP) AS nbGamesPlayed FROM d_gamesPlayed',
...
router.get('/nbQrScanned', function(req, res, next) {
  req.mysql.query(
    'SELECT COUNT(tokenTable) AS nbQrScanned FROM d_tableProf',
    function(error, results, fields) {
      if (error) {
...
router.get('/nbFilesAddedWeb', function(req, res, next) {
  req.mysql.query(
    'SELECT COUNT(idFile) AS nbFilesAddedWeb FROM d_files',
    function(error, results, fields) {
      if (error) {
        tools.dSend(
          res,
