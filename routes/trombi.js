...
router.post('/', function(req, res, next) {
  ...
  if (typeUser && idUser) {
    if (search) {
      if (typeUser == 1) {
        var myQuery =
          'SELECT DISTINCT e.idEleve, e.nomEleve, e.prenomEleve, e.picPath, c.idClasse, c.num, c.level FROM d_eleves AS e INNER JOIN d_classeEleve AS ce ON ce.idEleve = e.idEleve INNER JOIN d_classe AS c ON c.idClasse = ce.idClasse INNER JOIN d_profsAppClasse AS pa ON c.idClasse = pa.idClasse WHERE pa.idProf = ' +
          idUser +
          ' AND ((e.nomEleve LIKE "' +
          search +
          '%") OR (e.prenomEleve LIKE "' +
          search +
          '%")) ORDER BY e.nomEleve ASC';
      } else {
        var myQuery =
          'SELECT DISTINCT e.idEleve, e.nomEleve, e.prenomEleve, e.picPath, c.idClasse, c.num, c.level FROM d_eleves AS e INNER JOIN d_classeEleve AS ce ON ce.idEleve = e.idEleve INNER JOIN d_classe AS c ON c.idClasse = ce.idClasse INNER JOIN d_classeEcole AS cec ON cec.idClasse = c.idClasse INNER JOIN d_profsAppEcole AS pa ON cec.idEcole = pa.idEcole WHERE  pa.idProf = ' +
          idUser +
          ' AND ((e.nomEleve LIKE "' +
          search +
          '%") OR (e.prenomEleve LIKE "' +
      ...
      if (typeUser == 1) {
        var myQuery =
          'SELECT e.*, ce.idClasse, c.num, c.level FROM d_eleves AS e, d_classeEleve AS ce, d_classe AS c, d_profsAppClasse AS ac, d_users AS u WHERE u.idUser = ac.idProf AND ac.idClasse = c.idClasse AND c.idClasse = ce.idClasse AND ce.idEleve = e.idEleve AND u.idUser = ' +
          idUser +
          ' ORDER BY e.nomEleve ASC';
      } else {
        var myQuery =
          'SELECT e.*, ce.idClasse, c.num, c.level FROM d_eleves AS e, d_classeEleve AS ce, d_classe AS c, d_users AS u, d_classeEcole AS ce2, d_profsAppEcole AS ae WHERE u.idUser = ae.idProf AND ae.idEcole = ce2.idEcole AND ce2.idClasse = c.idClasse AND c.idClasse = ce.idClasse AND ce.idEleve = e.idEleve AND u.idUser = ' +
          idUser +
          ' ORDER BY e.nomEleve ASC';
      }
    }
...
router.post('/byClasse', function(req, res, next) {
  ...
  if (typeUser && idUser && idClasse) {
    if (search) {
      ...
      if (typeUser == 1) {
        var myQuery =
          'SELECT e.*, c.num, c.level FROM d_classe AS c, d_users AS u, d_profsAppClasse AS ac, d_classeEleve as ce, d_eleves as e WHERE u.idUser = ac.idProf AND ac.idClasse = c.idClasse AND c.idClasse = ce.idClasse AND ce.idEleve = e.idEleve AND u.idUser = ' +
          idUser +
          ' AND c.idClasse = ' +
          idClasse +
          ' ORDER BY e.nomEleve ASC';
        ...
        var myQuery =
          'SELECT e.*, c.num, c.level FROM d_eleves AS e, d_classeEleve AS ce, d_classe AS c, d_users AS u, d_classeEcole AS ce2, d_profsAppEcole AS ae WHERE u.idUser = ae.idProf AND ae.idEcole = ce2.idEcole AND ce2.idClasse = c.idClasse AND c.idClasse = ce.idClasse AND ce.idEleve = e.idEleve AND u.idUser = ' +
          idUser +
          ' AND c.idClasse = ' +
          idClasse +
          ' ORDER BY e.nomEleve ASC';
    ...
    tools.dSend(
      ...
      'byClasse',
      500,
      'idUser ou typeUser ou idClasse manquants.',
      null
    );
  }
});
